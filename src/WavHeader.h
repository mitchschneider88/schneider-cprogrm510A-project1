...
{
    const std::string chunkID {"RIFF"}; // RIFF Header - 4 bytes
    const std::string chunkSize {"----"}; // RIFF Chunk Size - size of file - 4 bytes
    const std::string format {"WAVE"}; // WAVE Header - 4 bytes
    const std::string subchunk1ID {"fmt "}; // FMT header, Subchunk1ID - 4 bytes
    const int subchunk1Size {16}; // Size of the fmt chunk - 4 bytes                    
    const int audioFormat {1}; // Audio format (1=PCM) - 2 bytes
    const int numChan {1}; // Number of channels 1=Mono 2=Sterio - 2 bytes                   
